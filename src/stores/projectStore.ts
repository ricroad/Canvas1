...
import { isDiscardedCanvasNodeType } from '@/features/canvas/domain/canvasNodes';
...
export interface Project extends ProjectSummary {
  ...
  scriptMd: string;
  scriptSourceFileName: string | null;
...
function encodePoolReference(
  ...
  const existingIndex = indexMap.get(value);
  if (typeof existingIndex === 'number') {
    return `${prefix}${existingIndex}`;
  }
...
function decodePoolReference(
  value: string | null | undefined,
  pool: string[] | undefined,
  prefix: string
): string | null | undefined {
  if (typeof value !== 'string' || !pool || !value.startsWith(prefix)) {
    return value;
  }
...
function mapNodeMediaReferences(
  nodes: CanvasNode[],
  mappers: {
    image: (imageUrl: string | null | undefined) => string | null | undefined;
    video: (videoUrl: string | null | undefined) => string | null | undefined;
  }
): CanvasNode[] {
  return nodes.map((node) => {
    ...
    if ('imageUrl' in nextData) {
      nextData.imageUrl = mappers.image(nextData.imageUrl as string | null | undefined) ?? null;
    }
    if ('previewImageUrl' in nextData) {
      nextData.previewImageUrl =
        mappers.image(nextData.previewImageUrl as string | null | undefined) ?? null;
    }
    if ('videoRef' in nextData) {
      nextData.videoRef = mappers.video(nextData.videoRef as string | null | undefined) ?? null;
    }
    if ('thumbnailRef' in nextData) {
      nextData.thumbnailRef =
        mappers.image(nextData.thumbnailRef as string | null | undefined) ?? null;
    }
    if ('_upstreamImageRef' in nextData) {
      nextData._upstreamImageRef =
        mappers.image(nextData._upstreamImageRef as string | null | undefined) ?? null;
    }
    ...
    if (Array.isArray(nextData.frames)) {
      nextData.frames = nextData.frames.map((frame) => {
        ...
        return {
          ...frameRecord,
          imageUrl: mappers.image(frameRecord.imageUrl as string | null | undefined) ?? null,
          previewImageUrl:
            mappers.image(frameRecord.previewImageUrl as string | null | undefined) ?? null,
        };
      });
    }
...
function mapHistoryMediaReferences(
  history: CanvasHistoryState,
  mappers: {
    image: (imageUrl: string | null | undefined) => string | null | undefined;
    video: (videoUrl: string | null | undefined) => string | null | undefined;
  ...
  return {
    past: history.past.map((snapshot) => ({
      ...snapshot,
      nodes: mapNodeMediaReferences(snapshot.nodes, mappers),
    })),
...
function encodeProject(project: Project): PersistedProject {
  ...
  const encodeImage = (imageUrl: string | null | undefined) =>
    encodePoolReference(imageUrl, imagePool, imageIndexMap, IMAGE_REF_PREFIX);
  const encodeVideo = (videoUrl: string | null | undefined) =>
    encodePoolReference(videoUrl, videoPool, videoIndexMap, VIDEO_REF_PREFIX);

  return {
    ...project,
    nodes: mapNodeMediaReferences(project.nodes, { image: encodeImage, video: encodeVideo }),
    history: mapHistoryMediaReferences(project.history, { image: encodeImage, video: encodeVideo }),
    imagePool,
    videoPool,
  };
}
...
function decodeProject(project: PersistedProject): Project {
  const decodeImage = (imageUrl: string | null | undefined) =>
    decodePoolReference(imageUrl, project.imagePool, IMAGE_REF_PREFIX);
  const decodeVideo = (videoUrl: string | null | undefined) =>
    decodePoolReference(videoUrl, project.videoPool, VIDEO_REF_PREFIX);

  return {
    ...project,
    nodes: mapNodeMediaReferences(project.nodes, { image: decodeImage, video: decodeVideo }),
    history: mapHistoryMediaReferences(project.history, { image: decodeImage, video: decodeVideo }),
  };
}
...
function extractPoolFromHistoryJson(historyJson: string, poolKey: string): string[] {
  const serializedKey = `"${poolKey}"`;
  const keyIndex = historyJson.indexOf(serializedKey);
  if (keyIndex < 0) {
    return [];
  }

  const arrayStart = historyJson.indexOf('[', keyIndex + serializedKey.length);
  if (arrayStart < 0) {
    return [];
  }
...
function extractImagePoolFromHistoryJson(historyJson: string): string[] {
  return extractPoolFromHistoryJson(historyJson, 'imagePool');
}

function extractVideoPoolFromHistoryJson(historyJson: string): string[] {
  return extractPoolFromHistoryJson(historyJson, 'videoPool');
}
...
function toProjectRecord(project: Project): ProjectRecord {
  ...
  return {
    ...
    historyJson: JSON.stringify({
      ...
      videoPool: encodedProject.videoPool ?? [],
    }),
    scriptMd: encodedProject.scriptMd ?? '',
    scriptSourceFileName: encodedProject.scriptSourceFileName ?? '',
    scriptImportedAt: encodedProject.scriptImportedAt ?? null,
    scriptAnalysisJson: encodedProject.scriptAnalysisJson ?? '',
  };
}

function discardRemovedCanvasNodes(
  nodes: CanvasNode[],
  edges: CanvasEdge[]
): { nodes: CanvasNode[]; edges: CanvasEdge[] } {
  const keptNodes = nodes.filter((node) => !isDiscardedCanvasNodeType(node.type));
...
function discardRemovedCanvasHistory(history: CanvasHistoryState): CanvasHistoryState {
  return {
    past: history.past.map((snapshot) => discardRemovedCanvasNodes(snapshot.nodes, snapshot.edges)),
    future: history.future.map((snapshot) => discardRemovedCanvasNodes(snapshot.nodes, snapshot.edges)),
  };
}
...
function fromProjectRecord(record: ProjectRecord): Project {
  ...
  const extractedVideoPool = extractVideoPoolFromHistoryJson(record.historyJson);
  ...
  const restoredCanvas = discardRemovedCanvasNodes(parsedNodes, parsedEdges);
  const parsedHistory = discardRemovedCanvasHistory({
  ...
  const persistedProject: PersistedProject = {
    ...
    scriptMd: record.scriptMd ?? '',
    scriptSourceFileName: record.scriptSourceFileName || null,
    scriptImportedAt: record.scriptImportedAt ?? null,
    scriptAnalysisJson: record.scriptAnalysisJson ?? '',
    imagePool: parsedHistoryPayload.imagePool ?? extractedImagePool,
    videoPool: parsedHistoryPayload.videoPool ?? extractedVideoPool,
  };

  const decodedProject = decodeProject(persistedProject);
  return {
    ...decodedProject,
    nodeCount: restoredCanvas.nodes.length,
...
interface ProjectState {
  ...
  setScriptMd: (scriptMd: string) => void;
  setScriptDocument: (sourceFileName: string | null, scriptMd: string) => void;
  setScriptAnalysisJson: (scriptAnalysisJson: string) => void;
}
...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  createProject: (name) => {
    ...
    const project: Project = {
      ...
      scriptMd: '',
      scriptSourceFileName: null,
      scriptImportedAt: null,
      scriptAnalysisJson: '',
    };
  ...
  setScriptMd: (scriptMd) => {
    const { currentProjectId, currentProject } = get();
    if (!currentProjectId || !currentProject || currentProject.id !== currentProjectId) {
      return;
    }

    if (currentProject.scriptMd === scriptMd) {
      return;
    }
  ...
  setScriptDocument: (sourceFileName, scriptMd) => {
    const { currentProjectId, currentProject } = get();
    if (!currentProjectId || !currentProject || currentProject.id !== currentProjectId) {
      return;
    }

    const normalizedSourceFileName = sourceFileName?.trim() || null;
    const hasChanged =
      currentProject.scriptMd !== scriptMd
      || currentProject.scriptSourceFileName !== normalizedSourceFileName;
    if (!hasChanged) {
      return;
    }
    ...
    const nextProject: Project = {
      ...currentProject,
      scriptMd,
      scriptSourceFileName: normalizedSourceFileName,
      scriptImportedAt: Date.now(),
      scriptAnalysisJson: '',
      updatedAt: Date.now(),
    };
  ...
  setScriptAnalysisJson: (scriptAnalysisJson) => {
    const { currentProjectId, currentProject } = get();
    if (!currentProjectId || !currentProject || currentProject.id !== currentProjectId) {
      return;
    }

    if (currentProject.scriptAnalysisJson === scriptAnalysisJson) {
      return;
    }
