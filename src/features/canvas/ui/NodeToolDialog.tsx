...
import {
  preloadSplitToolOptions,
  resolveToolDialogResult,
} from '@/features/canvas/application/toolDialogWorkflow';
...
export function NodeToolDialog() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const nextOptions = await preloadSplitToolOptions(sourceImageUrl, initialOptions);
        if (cancelled) {
          return;
        }
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      const resolvedResult = await resolveToolDialogResult(result);

      if (resolvedResult.kind === 'storyboardSplit') {
        const createdNodeId = addStoryboardSplitNode(
          sourceNode.id,
          resolvedResult.rows,
          resolvedResult.cols,
          resolvedResult.storyboardFrames,
          resolvedResult.frameAspectRatio
        );
        if (createdNodeId) {
          addEdge(sourceNode.id, createdNodeId);
        }
      } else if (resolvedResult.kind === 'exportImage') {
        const createdNodeId = addDerivedExportNode(
          sourceNode.id,
          resolvedResult.imageUrl,
          resolvedResult.aspectRatio,
          resolvedResult.previewImageUrl,
          {
