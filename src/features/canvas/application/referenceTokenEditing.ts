...
const REFERENCE_TOKEN_PREFIXES = ['图', 'img', '鍥'];
const DEFAULT_REFERENCE_TOKEN_PREFIX = '图';
...
function matchReferenceTokenPrefix(
  text: string,
  prefixStart: number
): { prefix: string; prefixLength: number } | null {
...
export function getReferenceImageLabel(index: number): string {
  return `${DEFAULT_REFERENCE_TOKEN_PREFIX}${index + 1}`;
}

export function buildReferenceToken(index: number): string {
  return `@${getReferenceImageLabel(index)}`;
}

export function stripReferenceTokenMarkerPrefix(text: string): string {
  let nextText = '';

  for (let index = 0; index < text.length; index += 1) {
    if (text[index] === '@') {
      const matchedPrefix = matchReferenceTokenPrefix(text, index + 1);
      if (
        matchedPrefix &&
        isAsciiDigit(text[index + 1 + matchedPrefix.prefixLength] ?? '')
      ) {
        continue;
      }
    }
...
export function findReferenceTokens(text: string, maxImageCount?: number): ReferenceTokenMatch[] {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    const digitsStart = index + 1 + matchedPrefix.prefixLength;
