...
import { Position, useUpdateNodeInternals, useViewport } from '@xyflow/react';
...
const STORYBOARD_NODE_HORIZONTAL_PADDING_PX = 32;
const STORYBOARD_GRID_GAP_PX = 2;
const STORYBOARD_GRID_BASE_CELL_HEIGHT_PX = 110;
const STORYBOARD_GRID_MAX_WIDTH_PX = 480;
const STORYBOARD_CONTROL_ROW_WIDTH_PX = 274;
const STORYBOARD_PARAMS_ROW_WIDTH_PX = 286;
const STORYBOARD_GEN_NODE_MIN_WIDTH_PX = 320;
const STORYBOARD_GEN_NODE_MIN_HEIGHT_PX = 400;
...
export const StoryboardGenNode = memo(({ id, data, selected, width, height }: StoryboardGenNodeProps) => {
  ...
  const paramFields = useMemo(
    ...
    [aspectRatioOptions, nodeData.extraParams, resolutionOptions, selectedAspectRatio, selectedModel, selectedResolution, t]
  );
  ...
  const buildPrompt = useCallback((): string => {
    ...
    const promptDirectives: string[] = [
      `Generate one ${gridRows}x${gridCols} storyboard sheet with ${gridRows * gridCols} panels`,
    ];
    if (storyboardGenKeepStyleConsistent) {
      promptDirectives.push('Keep the visual style consistent with the reference images');
    }
    if (storyboardGenDisableTextInImage) {
      promptDirectives.push('Do not render descriptive text inside the image');
    }
    ...
    frames.forEach((frame, index) => {
        ...
        if (!sanitizedDescription) {
          if (storyboardGenAutoInferEmptyFrame) {
            parts.push(`Panel ${index + 1}: infer the content from surrounding context`);
          }
          return;
        }

      parts.push(`Panel ${index + 1}: ${sanitizedDescription}`);
    });
  ...
  const handleGenerate = useCallback(async (previewGridOnly = false) => {
    ...
    const generationStartedAt = Date.now();
      ...
      const newNodeId = addNode(
        CANVAS_NODE_TYPES.imageResult,
        newNodePosition,
        {
          sourceGenNodeId: id,
          batchId,
          batchCreatedAt: generationStartedAt,
          snapshotParams: {
            prompt,
    ...
    try {
      ...
      const resolvedError = resolveErrorContent(generationError, 'Generation failed');
      const generationDebugContext: GenerationDebugContext = {
        sourceType: 'storyboardGen',
        providerId: selectedModel.providerId,
        requestModel: requestResolution.requestModel,
      ...
      void showErrorDialog(resolvedError.message, 'Error', resolvedError.details, reportText);
          ...
          nodeData.derivedFrom
            ? (
              <button
                ...
                onClick={(event) => {
                  event.stopPropagation();
                  setSelectedNode(nodeData.derivedFrom?.sourceResultNodeId ?? null);
                }}
              >
                {t('node.result.derivedFrom', { index: (nodeData.derivedFrom.derivedFromVariantIndex ?? 0) + 1 })}
              </button>
            )
            : undefined
        }
            ...
            return (
              ...
              >
                <div
                  ...
                  aria-hidden="true"
                  className="ui-scrollbar pointer-events-none absolute inset-0 overflow-y-auto overflow-x-hidden text-[11px] leading-[1.5] text-text-dark"
                ...
                <textarea
                  ...
                  wrap="soft"
                  className="ui-scrollbar nodrag nowheel relative z-10 h-full w-full resize-none overflow-y-auto overflow-x-hidden bg-transparent px-1.5 py-1 text-left text-[11px] leading-[1.5] text-transparent caret-text-dark placeholder:text-text-muted/40 focus:border-accent/50 focus:outline-none whitespace-pre-wrap break-words"
                  style={{ scrollbarGutter: 'stable' }}
                />
              </div>
            );
        ...
        <ModelParamsControls
          ...
          onSubmit={(event) => {
            const previewGridOnly =
              Boolean(enableStoryboardGenGridPreviewShortcut && event?.ctrlKey && event.altKey && event.shiftKey);
