...
#[tauri::command]
pub async fn save_source_to_directory(
    source: String,
    target_dir: String,
    suggested_file_name: Option<String>,
    extension: Option<String>,
    ...
    let (bytes, resolved_extension) = resolve_source_bytes(trimmed_source).await?;
    let output_extension = extension
        .as_deref()
        .map(normalize_extension)
        .filter(|value| !value.is_empty())
        .unwrap_or(resolved_extension);
    let dir_path = PathBuf::from(trimmed_dir);
    std::fs::create_dir_all(&dir_path)
        .map_err(|e| format!("Failed to create target dir: {}", e))?;
    ...
    std::fs::write(&output_path, bytes)
        .map_err(|e| format!("Failed to save source to target directory: {}", e))?;
