...
> 本文件与 `AGENTS.md` 共享绝大多数约束，是 Claude Code 的专属入口。**`AGENTS.md §12`（Git 作为检错与可追溯性的单一真相源）同样适用于本文件，以 `AGENTS.md §12` 为准**，详见下文第 12 节摘要。
...
## 12. Git 作为检错与可追溯性（与 AGENTS.md §12 同步）

完整条款见 `AGENTS.md §12`。以下为强约束摘要：

- **Commit 格式**：`<type>(<scope>): <why>`；一个 commit 只做一件事；标题写动机而非"update xxx"。
- **`fix` 类型必带 Trailer**（bug 索引 SSOT，不再另写 debug-log 文件）：
  ```
  Symptom: <现象>
  Trigger: <复现条件>
  Root-Cause: <一句话为什么>
  ...
  Affects: <文件列表>
  Verify: <验证步骤>
  Related: <可选，相关老 commit>
  ```
- **收尾流程**：`git status` → 按逻辑单元分别 commit → 返回 hash 给用户 → **不自动 push**。
- **检错标准流程**：
  1. 先 `git log --all --grep="<症状关键词>" --format="%h %s" | head -20`
  2. 命中 → `git show <old-hash>` 看 Trailer 与 diff → `git log <old-hash>..HEAD -- <file>` 看变化 → **读当前代码验证老 Root-Cause 是否仍成立**：成立参考老 Fix（新 commit 带 `Related: <old-hash>`）；不成立当作新 bug，**不得照搬**
  3. 不命中 → 回落 `git blame -L` / `git log --oneline -20` / `git log -S` / `git bisect`
- **Token 反模式**：禁止 `git log -p` 裸跑、禁止无 `-L` 的 `git blame`、禁止批量读 `docs/changelog/`、禁止未验证就套用老 fix。
- **文档分工**：bug 索引→git commit Trailer；叙事→`docs/changelog/`（可选）；发布→`docs/releases/`。

以 `AGENTS.md §12` 与上文冲突时：以 `AGENTS.md §12` 为准。以用户当次要求与本节冲突时：以用户要求为准。

---

## 13. Codex 任务下发规范

- **先动手再读文件**：每个独立模块（类型定义、store action、Rust 命令……）单独完成后立即 commit，不要把所有相关文件全部读完再统一动手。
- **原因**：context 积累过大时 OpenAI API 调用会无响应挂起，无 commit 的进度全部丢失。
- **强制约束**：所有交给 Codex 的 prompt 必须在"重要约束"部分包含："每完成一个独立模块就 commit 一次，不要把所有文件读完再动手。"
